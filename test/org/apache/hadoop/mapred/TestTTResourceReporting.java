...
public class TestTTResourceReporting extends TestCase {
  ...
  public static class FakeTaskScheduler extends JobQueueTaskScheduler {
    ...
    public List<Task> assignTasks(TaskTracker taskTracker)
      ...
      int cpuTemp = status.getResourceStatus().getCpuTemp();
      int diskTemp = status.getResourceStatus().getDiskTemp();
      float reportedCpuUsage = status.getResourceStatus().getCpuUsage();
	  float reportedDiskUsage = status.getResourceStatus().getDiskUsage();
  ...
  public void testResourceValuesOnLinux()
    ...
    conf.setLong("cpuTemp", plugin.getCpuTemp());
    conf.setLong("diskTemp", plugin.getDiskTemp());	
    conf.setLong("diskUsage", plugin.getDiskUsage());
    try {
      setUpCluster(conf);
